...
class GithubService {
    ...
    suspend fun loadTerraformFiles(repoInfo: GithubRepoInfo): List<String> = withContext(Dispatchers.IO) {
        val files = try {
            ...
            val apiUrl =
                "https://api.github.com/repos/${repoInfo.owner}/${repoInfo.repo}/contents/${repoInfo.path}?ref=${repoInfo.branch}"
        ...
        terraformFiles.forEach { file ->
            try {
                val rawUrl =
                    "https://raw.githubusercontent.com/${repoInfo.owner}/${repoInfo.repo}/${repoInfo.branch}/${file.path}"
